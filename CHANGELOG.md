# Changelog

All notable changes to `laravel-http-client-logger` will be documented in this file.

## v1.4.1 - 2026-02-24

### What's Changed

* fix: Исправлена ошибка логов, когда запрос завершился ошибкой by @popkovkirill in https://github.com/popkovkirill/laravel-http-client-logger/pull/20

**Full Changelog**: https://github.com/popkovkirill/laravel-http-client-logger/compare/v1.4.0...v1.4.1

## v1.4.0 - 2026-02-18

### What's Changed

* feat: Поддержка Laravel 10 by @popkovkirill in https://github.com/popkovkirill/laravel-http-client-logger/pull/17
* feat: Улучшение ContentTypeMessageFormatter by @popkovkirill in https://github.com/popkovkirill/laravel-http-client-logger/pull/18

**Full Changelog**: https://github.com/popkovkirill/laravel-http-client-logger/compare/v1.3.1...v1.4.0

## v1.3.1 - 2026-02-18

### What's Changed

* feat: Разные уровни логов в зависимости от кода ответа by @popkovkirill in https://github.com/popkovkirill/laravel-http-client-logger/pull/16

**Full Changelog**: https://github.com/popkovkirill/laravel-http-client-logger/compare/v1.3.0...v1.3.1

## v1.1.4 - 2026-02-18

feat: Разные уровни логов в зависимости от кода ответа#16

**Full Changelog**: https://github.com/popkovkirill/laravel-http-client-logger/compare/v1.1.3...v1.1.4

## v1.1.2 - 2024-04-23

Новый фишки для HeadersFormatter

## v1.1.1 - 2024-04-23

Теперь куки в заголовках скрываются
